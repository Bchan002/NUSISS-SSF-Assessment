...
COPY --from=builder \
    /app/target/noticeboard-0.0.1-SNAPSHOT.jar noticeboard.jar
...
HEALTHCHECK --interval=60s --timeout=30s --start-period=120s CMD curl -s -f http:/localhost:8080/status|| exit 1